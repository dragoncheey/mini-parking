...
- 录入页可调用本地识别 API，把拍照证据和定位字段交给模型识别，自动回填收费规则。
- 保存经纬度和入口备注；导航默认使用微信位置页。
- 详情页展示现场图片；需要导航时弹出微信位置页和复制坐标选项。
...
微信小程序不能任意拉起手机里的第三方原生 App。当前代码会弹出导航选项：

- `微信位置页`：调用 `wx.openLocation` 打开系统微信地图位置页。
- `复制坐标`：复制停车场名称、地址和经纬度，便于手动粘贴到其他地图。

`wx.openLocation` 失败时会自动复制地址和坐标。
